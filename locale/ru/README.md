<!--lint disable awesome-heading awesome-toc double-link-->

<div align="center" style="display: none;">
  <img src="/assets/banner.png" />
  <h1 align="center">Потрясающий Sunshine</h1>
  <h4 align="center">Коллекция потрясающих скриптов, инструментов, руководств и сопутствующего ПО для Sunshine</h4>
</div>

<div align="center" style="display: none;">
[
  <a href="#-Клиенты">Клиенты</a> •
  <a href="#%EF%B8%8F-Инструменты">Инструменты</a> •
  <a href="#-Игровые-магазины">Игровые магазины</a> •
  <a href="#-фронты">фронты</a> •
  <a href="#-Виртуальные-дисплеи">Виртуальные дисплеи</a> •
  <a href="#-скрипты">скрипты</a> •
  <a href="#-Тестирование">Тестирование</a> •
  <a href="#-Руководства">Руководства</a>
]
</div>

## 📺 Клиенты

### ✳️ Официальные клиенты Moonlight

- [Android](https://github.com/moonlight-stream/moonlight-android) - официальный клиент для Android.
- [ChromeOS](https://github.com/moonlight-stream/moonlight-chrome) - официальный ChromeOS-клиент.
- [iOS](https://github.com/moonlight-stream/moonlight-ios) - официальный iOS-клиент.
- [QT](https://github.com/moonlight-stream/moonlight-qt) - Официальный QT-клиент.
- [Embedded](https://github.com/moonlight-stream/moonlight-embedded) - официальный встроенный клиент.

### ✴️ Cторонние клиенты Moonlight

- [UWP/Xbox (One/Series)](https://github.com/TheElixZammuto/moonlight-xbox) - клиент UWP/Xbox (One/Series).
- [PS Вита](https://github.com/xyzz/vita-moonlight) - клиент для PS Vita.
- [Nintendo Switch](https://github.com/XITRIX/Moonlight-Switch) - клиент для Nintendo Switch.
- [Nintendo Wii U](https://github.com/GaryOderNichts/moonlight-wiiu) - клиент для Nintendo Wii U.
- [New Nintendo 3DS](https://github.com/zoeyjodon/moonlight-N3DS) - клиент для New Nintendo 3DS.
- [LG webOS TV](https://github.com/mariotaku/moonlight-tv) - клиент для LG webOS TV.
- [Samsung Tizen TV](https://github.com/brightcraft/moonlight-tizen) - клиент для Samsung Tizen TV.

## 🛠️ Инструменты

- [Cloudy Pad](https://github.com/PierreBeucher/cloudypad) - деплой Sunshine в Облаке: Paperspace, AWS, Azure, Google Cloud и других.
- [GSMS](https://github.com/LizardByte/GSMS) - Миграция игр и приложений с GeForce Experience на Sunshine.
- [LutrisToSunshine](https://github.com/Arbitrate3280/LutrisToSunshine) - этот скрипт отображает игры из Lutris, Heroic и/или Bottles, добавляет их в Sunshine и опционально загружает обложки из SteamGridDB.
- [NeonSunshine](https://github.com/NeonLightning/NeonSunshine) - добавляет и сортирует игры с обложками из SteamGridDB.
- [SunshineGameFinder](https://github.com/JMTK/SunshineGameFinder) - добавляет в Sunshine игры из распространённых путей установки.
- [sunshine_utils](https://github.com/designer-living/sunshine_utils) - утилиты для работы с игровыми стримингами Sunshine / Moonlight / Playnite.
- [vuinputd](https://github.com/joleuger/vuinputd) - включение удобной для использования в контейнерах обработки входных данных для Sunshine в Linux.

## 🎮 Игровые магазины

- [Epic Games](https://www.epicgames.com) - скачивайте и играйте на ПК в игры любого жанра. У них есть моды, DLC и бесплатные игры! Игры для всех.
- [GOG](https://www.gog.com) - скачивайте лучшие классические и новые игры на Windows, Mac & Linux. Огромный выбор игр без DRM, а также бесплатных дополнений.
- [Steam](https://store.steampowered.com) - идеальное место для игр, обсуждений и творчества.

## 💠 Frontends

- [LaunchBox](https://www.launchbox-app.com/) - эмулируйте, упорядочивайте и украшайте свою коллекцию игр.
- [Playnite](https://github.com/JosefNemec/Playnite) - менеджер библиотек видеоигр с поддержкой множества сторонних плагинов, поддержкой эмуляции игр и единым интерфейсом для ваших игр.
- [Steam](https://store.steampowered.com) - идеальное место для игр, обсуждений и творчества.

## 💻 Виртуальные дисплеи

- [evdi](https://github.com/DisplayLink/evdi) - модуль ядра Linux, позволяющий управлять несколькими экранами. По сути, это виртуальный дисплей, который можно добавлять, удалять и получать обновления экрана.
- [krfb](https://invent.kde.org/network/krfb) - утилита командной строки KDE Plasma (Wayland), которая создает бесголовной виртуальный дисплей, доступный по сети через VNC.
- [parsec-vdd](https://github.com/nomi-san/parsec-vdd) - автономный виртуальный дисплей Parsec.
- [Virtual-Display-Driver](https://github.com/itsmikethetech/Virtual-Display-Driver) - добавьте виртуальные мониторы к вашему устройству на Windows 10/11! Работает с VR, OBS, Sunshine и любым другим ПО для общего доступа к рабочему столу.
- [VirtualDisplayDriver_Wizard](https://github.com/sofmeright/VirtualDisplayDriver_Wizard) - GUI инструмент, который может интегрироваться с другими программами вроде Sunshine для эффективной работы с реализациями Indirect Display Driver Sample (IddSample).
- [virtual-display-rs](https://github.com/MolotovCherry/virtual-display-rs) - драйвер виртуального дисплея Windows для добавления нескольких виртуальных мониторов в ваш ПК! Для Win10+. Работает с VR, OBS, стриминговым ПО и т.д.

## 📜 Скрипты

- [Dummy Plug Automation](https://github.com/XenHat/dummy-plug-automation) - автоматизация стриминга с использованием dummy-заглушки в Linux.
- [FrlToggle](https://github.com/FrogTheFrog/frl-toggle) - ограничитель FPS для Nvidia.
- [GsyncToggle](https://github.com/FrogTheFrog/gsync-toggle) - переключатель Nvidia G-Sync.
- [VsyncToggle](https://github.com/xanderfrangos/vsync-toggle) - переключатель Nvidia V-Sync.
- [Windows Display Automation](https://github.com/fehbari/sunshine-scripts) - автоматизирует настройки отображения, такие как подключение виртуального дисплея, настройка разрешения, частоты обновления, HDR, G-Sync или ограничение FPS для оптимального стриминга.

## 🧪 Тестирование

- [LizardByte/Gamepad-Tester](https://app.lizardbyte.dev/gamepad-tester) - Official тестер геймпадов от LizardByte.
- [HardwareTester/GamepadTester](https://hardwaretester.com/gamepad) - этот инструмент показывает текущее состояние ваших геймпадов, джойстиков и других устройств, поддерживающих HTML5 Gamepad API.

## 📓 Руководства

- [Документация LizardByte](https://docs.lizardbyte.dev/projects/sunshine) - официальная документация по Sunshine.
- [Блог LizardByte](https://app.lizardbyte.dev/blog) - блог LizardByte, содержащий официальные руководства и рекомендации сообщества по Sunshine.

## Внести вклад

Помощь приветствуется! Но, пожалуйста, следуйте инструкциям LizardByte
[Вклад в руководство](https://docs.lizardbyte.dev/latest/developers/contributing.html)
при участии в этом проекте.
